...
export class LivroGeneroService {
  ...
  async create(createLivroGeneroDto: CreateLivroGeneroDto): Promise<LivroGenero> {
  ...
  async findAll(): Promise<LivroGenero[]> {
    return await this.livroGeneroRepository.find({ relations: ['livro', 'genero'] });
  }
  ...
  async findOne(id: string): Promise<LivroGenero> {
    ...
    if (!livroGenero) throw new NotFoundException('Relação Livro-Gênero não encontrada');
    return livroGenero;
  }
